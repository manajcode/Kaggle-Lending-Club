...
library(pacman)
p_load(tidyverse, tinytex, Amelia, knitr, DataExplorer, imputeMissings, corrplot, magrittr)
```
...
Here I apply a process to remove columns that provide no information in the ability to classify loan borrowers as risky or safe. The steps are as follows:
    ...
    d. Identify the variables that contain numeric or integer data but have too little variability.

    e. Variables that have data that cannot be analyzed (e.g. columns with notes, aka "empty values")

    f. remove "loan status", the original response variable that was transformed into a binary classifier.
    
    g. remove any date columns within the data set; not helpful, similar to c.
    
    h. Identify the variables that contain a high degree of correlation with 1 or more other variables.


### 4.a.1) Evaluate the percent of NAs in the predictor variables.

To begin, I identify the percent frequency of NAs per each predictor variable, and specifically present the variables whose percentage of NAs is greater than 60%. In general a variable whose missing more than half of its data cannot be used to predict an outcome, as the probability of selecting a values is by default already below 50%. I select a slightly higher percentage than 50% to select variables that can provide better information and power in their ability to classify outcomes. 
...
Given this information, we can now filter out the variables whose percentage of NAs is greater than 60%.
...
For duplicates where the variables are floating and integer, the value kept depends on the nature of the variable. In this particular data set, floating variables will most likely represent financial (money) data; either choice can be accepted, however it may be easier to work with the integer values.
...
I first identify the columns that are of type factorand count the number of levels of each predictor variable.
...
I then keep only the variables with less than 7 levels. Notice that in this step I remove the original, unmodified predictor Loan_status, saving us a step later on.
...
### 4.d) Identify the variables that contain numeric or integer data but have too little variability.

First I identify the variables that are numeric, and then check the status of nas within them; calculating variance on columns with NAs will only return NA.
```{r}
...
Here I analyze the variables of type integer; there is only one and it is a variable containing year.
```{r}
LCTF10 %>%
...
### 4.e) Variables that have data that cannot be analyzed.



### 4.f) Remove loan_status.
...
Some variables are nearly identical except for an added "_int" or "_inv". "_int" stands for interest, and "_inv" stands for "investment" or "invested" usually in repsonse to an outside lender's investment.


```{r}
# pull out the predictor variable names.
b<- colnames(LCTF10)

# detect which columns contain the key phrases.
which(str_detect(b, pattern="_inv"))
which(str_detect(b, pattern = "_int"))

b[c(3,37,39,41)]
```

as we can see there are four variables that contain our key words. A search for these columns shows:

```{r}
head(LCTF10$funded_amnt)
head(LCTF10$funded_amnt_inv)

head(LCTF10$out_prncp_inv)
head(LCTF10$out_prncp)

head(LCTF10$total_pymnt_inv)
head(LCTF10$total_pymnt)
